...
if args.ip_expr[len(args.ip_expr)-1] == '!':
  res = re.findall(r'[0-9\.]+|[^0-9\.]+', args.ip_expr)
  res = res[:-1]
  res[len(res)-1] = 'factorial(' + res[len(res)-1] + ')'
  args.ip_expr = "".join(str(x) for x in res)
try:
  flag = False
